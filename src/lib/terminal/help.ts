    ...
    output.update((prev) => [
        ...prev,
        {
            ...
            res: [
                ...
                "",
                "Commands:",
                ...Object.keys(cmds).map(
                    (cmd) =>
                        `  <span class="text-cyan-400">${cmd.padEnd(maxLength)}</span> ${cmds[cmd].help}`
                ),
                "",
                "Options:",
                ...Object.entries(uniqueOpts).map(([opt, help]) => {
                    const coloredOpts = opt
                        .split(", ")
                        .map((o) => `<span class="text-cyan-400">${o}</span>`)
                        .join(", ");
